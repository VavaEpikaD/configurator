...
Release: `20260908-contact-5` (Step 5: shared contact shading)

This is the shared material, quality, geometry and rendering foundation for **Window and Pergola only**. This update adds a scene-owned contact-occlusion stage without changing the accepted corrective material presets, texture generators, HDR probe, exposure or product geometry. See `CONTACT_SHADING.md` for the rendering contract, budgets, safeguards and acceptance checks. Earlier edge/geometry work remains in place. Fabrication rules, finish selections, pricing, saved-configuration schemas and account/cart code are unchanged.
...
| Contact shading | Disabled | 12 samples; half-resolution | 20 samples; three-quarter-resolution |
| Contact-buffer longest edge | No buffers | Up to 960 px | Up to 1440 px |
...
Contact shading now modulates indirect lighting on eligible opaque PBR surfaces. Compact contact buffers are capped at 720 px. The original beauty pass still owns antialiasing, glass transmission, direct shadows, tone mapping, output conversion and UI rendering. There is no screen-space reflection, bloom, path tracing or lighting/material retune in this update. Generated edge finishes remain documented separately in `EDGE_FINISHES.md`.
...
// In the host animation/capture loop, instead of renderer.render(scene, camera):
...
Apply this update at the **repository root**, over the accepted corrective update (`configurator_material_shading_pass_20260908_update.zip`), with the earlier materials/geometry/edge updates already applied. The ZIP contains only changed/new source, validation and documentation files, plus `commit_message.md` at its root. It contains no dependencies, prebuilt site, font files or existing customer content.
...
For this step, expect subtle shading at frame recesses, handle/backplate contact and Pergola post/deck contact. The previous restrained coating grain and edge highlights remain unchanged. The `contactShading` diagnostic block reports whether the new effect actually ran: Balanced/High should show `status: "active"`, `error: null` and two allocated targets on a supported WebGL2 renderer. Low reports `quality-disabled` and zero targets. The geometry sub-version remains `20260908-corrective-4` because this step does not change geometry. Pay particular attention to resizing, mixed fixed/opening Window layouts, trans-mullions, exploded view, Pergola side closures and switching between presets. Supplied Window CAD profiles remain exact. See `EDGE_FINISHES.md` for the targeted parts.
